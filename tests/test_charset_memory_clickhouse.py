...
def store():
    ...
    s.client.command(
        f"INSERT INTO {s.database}.events ({columns}) "
        f"SELECT generateUUIDv4(number), '{_CASE}', '{_SOURCE}', 'u_ex.log', number, number, "
        "repeat('0', 64), repeat('0', 64), 'iis', '1', now64(3), 'GET', "
        f"if(number < {_PLAIN}, "
        ...
        f"'', repeat('0', 64) FROM numbers({_PLAIN + 2})"
    )
    yield s
...
def test_grouped_self_baseline_learn(bounded_scan, store, monkeypatch):
    ...
    monkeypatch.setattr(_scan, "detect_scan_memory_budget", lambda: 2 * _CAP)
    svc = StatisticalAnomalyService(clickhouse=store)
