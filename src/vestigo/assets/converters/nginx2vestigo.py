...
def convert(
    ...
    log_types = sorted({log_type for _path, log_type in files})
    metadata = {
        ...
        META_TIMEZONE_ASSUMPTION: (
            "access/redirect timestamps honor their own offset; error-log "
            "timestamps have no timezone and are assumed UTC"
        ),
        META_PARSE_DECISIONS: json.dumps(
            {"log_types": log_types, "since": since, "until": until}, sort_keys=True
        ),
    }
        ...
        for path, log_type in files:
            ...
            if parallel:
                parsed, skipped, skipped_by_time = _convert_file_parallel(
                    path, log_type, hashes[path], buffer, workers, verbose, since_dt, until_dt
                )
            else:
