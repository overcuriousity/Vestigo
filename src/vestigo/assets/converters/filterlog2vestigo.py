...
def convert(
    ...
    year_used = year if year is not None else datetime.datetime.now(datetime.UTC).year
    metadata = {
        ...
        META_TIMEZONE_ASSUMPTION: (
            f"BSD syslog timestamps assumed year={year_used}, UTC; ISO-8601 "
            "timestamps honor their own offset (naive assumed UTC)"
        ),
        META_PARSE_DECISIONS: json.dumps(
            {"year": year_used, "since": since, "until": until}, sort_keys=True
        ),
    }
        ...
        for path in files:
            ...
            if parallel:
                parsed, skipped, skipped_by_time = _convert_file_parallel(
                    path, hashes[path], buffer, workers, year, verbose, since_dt, until_dt
                )
            else:
