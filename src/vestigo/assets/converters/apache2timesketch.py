...
# logged with a best-effort address. The `(?:,\s*\S+)*` tail only engages on a
# literal comma, so an ordinary single-token %h still matches exactly as before.
...
# format's does (see nginx._ACCESS_LOG_RE): a proxy header logged in %h's place
# otherwise shifted every following group and dropped the line entirely.
_CLF_RE = re.compile(
    r'(\S+(?:,\s*\S+)*) (\S*) (\S*) \[([^\]]+)\] "([^"]*)" (\d+) (\S+)\s*$'
)
