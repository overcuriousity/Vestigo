#!/usr/bin/env python3
# apache2timesketch.py — self-contained converter, vendored from https://github.com/overcuriousity/2timesketch
...
#
# Apache HTTP Server access (combined/common, other_vhosts) and error logs (2.4 and 2.2 formats) to Timesketch timeline.
...
#
# The first group is the %h slot. It is deliberately more than `\S+`: a server
# fronted by a proxy is often configured to log `%{X-Forwarded-For}i` there, and
# that header is a comma-joined chain ("192.0.2.1, 198.51.100.2"). With a bare
# `\S+` the chain shifted every following group by one and the line matched
# nothing at all — the whole request was dropped as unparseable rather than
# logged with a best-effort address.
#
# Each chain element is `[^\s,]+`, never `\S+`. `\S` matches a comma, so
# `\S+(?:,\s*\S+)*` lets the element and the separator claim the same
# characters, and a line of repeated "!," with no match at the end backtracks
# exponentially — roughly 4x per two extra repetitions, days of CPU from an
# ~80-byte line. The input is a web server's access log, i.e. written by
# whoever sends the request. Excluding the comma from the element leaves exactly
# one way to split a chain, which keeps the match linear. `_CLF_RE` in apache.py
# carries the same slot and must keep the same shape.
...
def _parse_access_line(line: str, log_type: str, source_file: str) -> dict[str, Any] | None:
    ...
    # %h is documented as "remote hostname (or IP address)": with
    # HostnameLookups on, or a proxy header logged in its place, it is not an
    # address at all. normalize_ip() then returns "" — so the token has to be
    # kept under its own column, or the only record of who made the request is
    # thrown away and src_ip is empty with nothing on the row to explain it.
    src_ip = normalize_ip(ip)
    if not src_ip and "," in ip:
        # An X-Forwarded-For chain logged in the %h slot: the leftmost entry is
        # the original client, the rest are the proxies it traversed.
        src_ip = first_ip(ip)
...
_NGINX_EXTRA_FIELDS_BY_TYPE = _EXTRA_FIELDS_BY_TYPE
...
# The %h slot accepts a comma-joined chain for the same reason the combined
# format's does, and with the same comma-free element: see nginx._ACCESS_LOG_RE
# for why `\S+` there is an exponential-backtracking bug on attacker-written
# input, not a style choice.
_CLF_RE = re.compile(
    r'([^\s,]+(?:,\s*[^\s,]+)*) (\S*) (\S*) \[([^\]]+)\] "([^"]*)" (\d+) (\S+)\s*$'
)
...
def _parse_apache_access_line(
    ...
    # A first parse that landed a real address is the answer. One that did not
    # is *not* good enough to stop at while the line still carries a vhost
    # prefix: a LogFormat that drops %l ("%v %h %u ...") presents three tokens
    # before the timestamp just like plain "%h %l %u", so the regex matches with
    # the **vhost** in the address slot — src_ip comes out empty, the real
    # client lands in remote_ident, and nothing counts the line as unparseable.
    # Re-try without the prefix and keep whichever attempt found an address.
    if row is not None and row.get("src_ip"):
        return row

    vhost_match = _VHOST_PREFIX_RE.match(line)
    if vhost_match:
        stripped = line[vhost_match.end():]
        alt = _parse_apache_access_line_no_vhost(stripped, source_file)
        if alt is not None and (row is None or alt.get("src_ip")):
            alt["vhost"] = vhost_match.group(1)
            return alt
...
def _parse_apache_access_line_no_vhost(
    ...
    # Same contract as the combined-format branch in nginx._parse_access_line:
    # %h is "remote hostname (or IP address)", so the raw token is kept under
    # client_host and src_ip is populated only when it really is an address.
    src_ip = normalize_ip(ip)
    if not src_ip and "," in ip:
        src_ip = first_ip(ip)
...
def _split_client(client: str) -> tuple[str, str, int | None]:
    ...
    Returns ``(src_ip, client_host, port)``. ``client_host`` is the host token
    exactly as Apache logged it and is always populated; ``src_ip`` is the
    canonical address, or ``""`` when the token is a hostname — an error log
    written with HostnameLookups on records one, and keeping only the empty
    address would lose the sole record of who triggered the error.
    """
    if client.startswith("["):
        ...
        if end != -1:
            ...
            return normalize_ip(host), host, port
        return normalize_ip(client), client, None

    if client.count(":") == 1:
        head, _, tail = client.rpartition(":")
        if tail.isdigit() and head:
            return normalize_ip(head), head, int(tail)
...
def _parse_error_line(line: str, source_file: str) -> dict[str, Any] | None:
    ...
    if client:
        src_ip, client_host, src_port = _split_client(client)
        row["src_ip"] = src_ip
        row["client_host"] = client_host
