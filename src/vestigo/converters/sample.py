...
The budget is deliberately small (``converter_sample_bytes``, default 4 KiB —
a few dozen lines). The excerpt teaches the model a *format*, not a story, and
a log is mostly the same handful of lines over and over: 64 KiB of one nginx
access log was ~50k tokens of prompt that a local model could not finish
inside its server's cap, and every fact the converter needed sat in the first
few lines. Generated converters are best effort by design — a format the
excerpt does not show is what the saved-converter route is for.
"""
...
def sample_as_file(sample: Sample, dest_dir: Path, filename: str) -> Path:
    """Write the excerpt as ``dest_dir/<basename(filename)>`` — the sample-phase input.

    Every block, not just the head: the excerpt is a few KiB, and running the
    guarded script over the middle and the tail as well is how the sample phase
    gets to see a format that changed mid-file or the newest timestamps before
    the full run does.
