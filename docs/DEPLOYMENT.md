...
`max_threads` is per-scan thread width. At `VESTIGO_STAT_SCAN_MAX_THREADS=0` (default) it is
`detected_cores ÷ concurrency`, floor 2, where `detected_cores` is what ClickHouse resolves
