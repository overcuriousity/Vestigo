...
export interface Annotation {
  id: string;
  event_id: string;
  annotation_type: "comment" | "tag" | "outlier";
  content: string;
  created_at: string;
  created_by?: string;
  /** "user" (human) | "system" (machine-generated by analysis pipeline) */
  origin?: "user" | "system";
  /** Structured math details; only present on system annotations. */
  details?: {
    method: string;
    distance: number;
    rank: number;
...
export type VectorStatus = "ok" | "not_embedded" | "insufficient_vectors" | "vector_not_found";
...
export interface TagAnomaliesResponse {
  status: VectorStatus;
  tagged: number;
  sample_size?: number;
  embedding_config_hash?: string;
...
// Similarity search: find events similar to a given event.
...
// Outlier detection: surface the most unusual events in a timeline.
...
// Tag outliers: persist system annotations for the top-N unusual events.
export async function tagAnomalies(
  caseId: string,
  timelineId: string,
  limit = 50,
  sampleSize = 5000,
): Promise<TagAnomaliesResponse> {
  const response = await api.post(
    `/api/cases/${caseId}/timelines/${timelineId}/anomalies/tag`,
    { limit, sample_size: sampleSize },
  );
  return response.data as TagAnomaliesResponse;
}
